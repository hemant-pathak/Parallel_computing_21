...
int main(int argc, char* argv[])
{
  ...
  // np -> no. of processes , pid -> process id
	int pid, np, elements_per_process, n_elements_recieved;
	MPI_Status status;
	MPI_Init(&argc, &argv);

	// find out process ID,  and # processes were started
	...
	if (pid == 0) {
		...
		// checking if more than 1 processes are run
		if (np > 1) {
			// distributes the portion of array to child processes 
			// to calculate their partial sums
			for (i = 1; i < np - 1; i++) {
	...
	// clear MPI state before exit of process
	MPI_Finalize();
